...
static void continueStatement(); // Forward declaration
...
// Save outer loop
  ...
  // The jump target for continue (defaults to start if no increment)
  ...
  if (!match(TOKEN_RIGHT_PAREN)) {
    ...
    // Continue in a for loop goes to the increment
...
static void switchStatement() {
  ...
  int state = 0; // 0: before cases, 1: in cases, 2: after default
  int caseEnds[256]; // To track jumps to the end of the switch
  int caseCount = 0;
  ...
  while (!check(TOKEN_RIGHT_BRACE) && !check(TOKEN_EOF)) {
    if (match(TOKEN_CASE) || match(TOKEN_DEFAULT)) {
      ...
      if (state == 1) {
        // Jump over the next case's comparison if falling through
      ...
      if (caseType == TOKEN_CASE) {
        state = 1;
        emitByte(OP_DUP); // Duplicate switch value for comparison
        expression();
  ...
  // Final patches
  if (previousCaseSkip != -1) patchJump(previousCaseSkip);
  emitByte(OP_POP); // Pop the original switch value
...
static void continueStatement() {
  ...
  // Pop locals inside loop body up to the loop scope depth
