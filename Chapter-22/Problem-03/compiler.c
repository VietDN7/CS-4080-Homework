...
static void namedVariable(Token name, bool canAssign) {
  ...
  if (arg != -1) {
    getOp = OP_GET_LOCAL;
    setOp = OP_SET_LOCAL;
    isConst = current->locals[arg].isConst; // Check local const status
  } else if ((arg = resolveUpvalue(current, &name)) != -1) {
    getOp = OP_GET_UPVALUE;
    setOp = OP_SET_UPVALUE;
    // For upvalues, you'd check the original local's isConst
  } else {
    arg = identifierConstant(&name);
    getOp = OP_GET_GLOBAL;
    setOp = OP_SET_GLOBAL;
    isConst = isConstGlobal(&name); // Check global const status
  }
