...
ObjString* copyString(const char* chars, int length) {
  ...
  ObjString* string = makeString(length); // Single allocation
  string->hash = hash;
  
  memcpy(string->chars, chars, length); // Copy into the flexible array
  string->chars[length] = '\0'; // Null terminate for C-string compatibility
