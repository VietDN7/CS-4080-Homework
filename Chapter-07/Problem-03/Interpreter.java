...
//> binary-plus
      case PLUS:
        // If both are numbers, perform standard addition
        if (left instanceof Double && right instanceof Double) {
          return (double)left + (double)right;
        }

        // Extension: if either is a string, coerce both to strings and concatenate
        if (left instanceof String || right instanceof String) {
          return stringify(left) + stringify(right);
        }

        // If neither condition is met, throw the error
        throw new RuntimeError(expr.operator,
            "Operands must be two numbers or at least one must be a string.");
...
//< binary-plus
      ...
      case SLASH:
        ...
        // Check if the divisor (right) is zero
        if ((double)right == 0) {
