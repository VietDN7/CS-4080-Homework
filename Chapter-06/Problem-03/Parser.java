...
//> expression
  private Expr expression() {
    return comma(); // Changed from assignment()
  }
//< expression

  private Expr comma() {
    Expr expr = conditional(); // Changed from assignment()

    while (match(COMMA)) {
      Token operator = previous();
      Expr right = conditional(); // Changed from assignment()
  ...
  private Expr conditional() {
    ...
    if (match(QUESTION)) {
      // The middle part can be any expression
      Expr thenBranch = expression(); 
      consume(COLON, "Expect ':' after then branch of conditional expression.");
      
      // Calling conditional() recursively for the else branch
...
//> unary
  private Expr unary() {
    // Error production: check forbinary operators at the start of an expression
    if (match(COMMA, BANG_EQUAL, EQUAL_EQUAL, GREATER, GREATER_EQUAL, LESS, LESS_EQUAL, PLUS, SLASH, STAR)) {
      error(previous(), "Binary operator can't be used without a left-hand operand");
      unary(); // Consume the right-hand operand to recover
      return null;
    }
