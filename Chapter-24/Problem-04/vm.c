...
static bool typeNative(int argCount, Value* args, Value* result) {
  ...
  if (IS_NUMBER(value)) {
    ...
    *result = OBJ_VAL(copyString("boolean", 4));
    ...
    *result = OBJ_VAL(copyString("object", 7));
  }
}
...
//> Calls and Functions interpret-call
      case OP_CALL: {
        ...
        // Save current local ip back to the frame before calling
        frame->ip = ip; 
        ...
        // Load the new frame and its ip into the local variable
