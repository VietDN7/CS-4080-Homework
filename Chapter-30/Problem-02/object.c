...
Value copyString(const char* chars, int length) {
  // If small enough, pack it into the Value instead of allocating an Obj
  if (length <= 7) {
    Value value;
    value.type = VAL_SMALL_STRING;
    value.as.smallString.length = (uint8_t)length;
    memcpy(value.as.smallString.data, chars, length);
    value.as.smallString.data[length] = '\0'; // Null terminate
    return value;
  }
