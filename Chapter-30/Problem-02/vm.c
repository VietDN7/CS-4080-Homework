...
static void concatenate() {
  ...
  const char* bChars = IS_SMALL_STRING(valB) ? AS_SMALL_STRING(valB) : AS_STRING(valB)->chars;
  int bLen = IS_SMALL_STRING(valB) ? SMALL_STRING_LEN(valB) : AS_STRING(valB)->length;
  
  const char* aChars = IS_SMALL_STRING(valA) ? AS_SMALL_STRING(valA) : AS_STRING(valA)->chars;
  int aLen = IS_SMALL_STRING(valA) ? SMALL_STRING_LEN(valA) : AS_STRING(valA)->length;

  int length = aLen + bLen;

  if (length <= 7) {
    char data[8];
    memcpy(data, aChars, aLen);
    memcpy(data + aLen, bChars, bLen);
    data[length] = '\0';
    pop(); pop();
    push(copyString(data, length)); // This will return a VAL_SMALL_STRING
    return;
  }
...
//> Strings add-strings
      case OP_ADD: {
        if ((IS_STRING(peek(0)) || IS_SMALL_STRING(peek(0))) && 
            (IS_STRING(peek(1)) || IS_SMALL_STRING(peek(1)))) {
          concatenate();
