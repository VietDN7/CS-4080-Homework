...
static bool findBetaMethod(ObjClass* klass, ObjString* name, Value* method, ObjClass** owner) {
    // 1. Find the root of the chain
    ObjClass* root = klass;
    while (root->superclass != NULL) root = root->superclass;

    // 2. Walk down from root to 'klass' to find the FIRST implementation
    ObjClass* current = root;
    while (current != NULL) {
        if (tableGet(&current->methods, name, method)) {
            if (owner) *owner = current;
            return true;
        }
        // Logic to move 'down' towards the receiver's class...
    }
    return false;
}
...
//> Superclasses interpret-inherit
      case OP_INHERIT: {
        ...
        subclass->superclass = AS_CLASS(superclass); // Store the link
        pop(); // Subclass.
        break;
      }
//< Superclasses interpret-inherit
...
//< Methods and Initializers interpret-method
      ...
      case OP_INNER: {
        ObjString* methodName = frame->closure->function->name;
        ObjInstance* instance = AS_INSTANCE(frame->slots[0]); // 'this'
        
        // Search subclasses: start from receiver, walk up until we hit the class 
        // immediately below the current frame->classOwner.
        ObjClass* search = instance->klass;
        ObjClass* nextImplementation = NULL;

        while (search != NULL && search != frame->classOwner) {
            Value v;
            if (tableGet(&search->methods, methodName, &v)) {
                nextImplementation = search; 
            }
        ...
        if (nextImplementation != NULL) {
            Value method;
            tableGet(&nextImplementation->methods, methodName, &method);
            // Call it!
            call(AS_CLOSURE(method), 0); 
            frame = &vm.frames[vm.frameCount - 1];
            frame->classOwner = nextImplementation; 
        }
        break;
      }
